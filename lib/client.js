window.__ModuleLoader__.load({ id: 'dsh-ssh-servers', factory: (require) => {
  ...
  const chipStyle = {
    ...
    border: '1px solid var(--border-color, #d8d8d8)', borderRadius: 999,
  }
  ...
  // never rendered back.
  function LoginPanel({ profiles, audit, connections, onClose, onChanged }) {
    ...
    const submit = async () => {
      ...
      try {
        ...
        setPassword('')
        // The panel stays open on purpose: logging in to a second server must
        // not require disconnecting the first one.
        setNotice(`已连接 ${profile.label}`)
        await onChanged()
      } catch (cause) {
        setPassword('')
        setError(cause instanceof Error ? cause.message : String(cause))
    ...
    return React.createElement('div', { style: cardStyle },
      React.createElement(Field, { label: '服务器' },
        React.createElement('select', {
          ...
          const connected = connections.some((live) => live.profileId === entry.id)
          return React.createElement('option', { key: entry.id, value: entry.id },
            `${entry.label} — ${entry.username}@${entry.host}:${entry.port}${connected ? '（已连接）' : ''}`)
        })),
      ),
      profile !== undefined
      ...
      notice !== null ? React.createElement('div', { style: hintStyle }, notice) : null,
      React.createElement('div', { style: { display: 'flex', gap: 8, justifyContent: 'flex-end' } },
        React.createElement('button', { style: buttonStyle(false), disabled: busy, onClick: () => onClose() }, '关闭'),
  ...
  function StatusDock() {
    ...
    const disconnectOne = async (profileId) => {
      setBusyId(profileId)
      try {
        await post('/disconnect', { profileId })
      } catch {
        // The refresh below still runs, so the bar keeps showing the truth.
      }
      setBusyId(null)
      await refresh()
    }
    ...
    return React.createElement('div', null,
      React.createElement('div', { style: barStyle },
        ...
        React.createElement('span', { style: { opacity: 0.85 } },
          live.length === 0 ? '未连接' : `已连接 ${live.length} 台`),

        // One chip per live connection, each with its own disconnect. Several
        // servers may be logged in at once — SshManager keys connections by
        // profile and only reconnecting the SAME profile replaces it — so the
        // only thing that used to forbid it was this bar hiding its login
        // button as soon as any connection existed.
        ...live.map((entry) => React.createElement('span', {
          key: entry.profileId,
          style: chipStyle,
          title: `远程工作目录：${entry.remoteCwd}`,
        },
          React.createElement('span', { style: dotStyle(true) }),
          React.createElement('span', { style: { fontWeight: 600 } }, entry.label),
          React.createElement('span', { style: monoStyle }, `${entry.username}@${entry.host}`),
          React.createElement('button', {
            style: chipButtonStyle,
            disabled: busyId === entry.profileId,
            title: `断开 ${entry.label}`,
            onClick: () => void disconnectOne(entry.profileId),
          }, busyId === entry.profileId ? '…' : '×'),
        )),

        React.createElement('span', { style: { flex: 1 } }),
        React.createElement('button', {
          style: buttonStyle(live.length === 0),
      ...
      open
        ? React.createElement(LoginPanel, {
            profiles: state.profiles, audit: state.audit, connections: live,
